...
CREATE DATABASE /*!32312 IF NOT EXISTS*/ `japedidos` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`10.0.0.%` PROCEDURE `select_estados_pedido`(IN id_ped INT)
    READS SQL DATA
BEGIN
...
/*!50003 SET collation_connection  = utf8mb4_general_ci */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`10.0.0.%` PROCEDURE `select_pedidos_by_estado`(IN id_est INT)
    READS SQL DATA
BEGIN
...
DELIMITER ;;
CREATE DEFINER=`root`@`10.0.0.%` PROCEDURE `select_produtos_pedido`(IN id_ped INT)
    READS SQL DATA
BEGIN
